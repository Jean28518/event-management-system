...
class LectureForm(forms.Form):
    # event = forms.ModelChoiceField(label="Event", queryset=Event.objects.all(), widget=forms.Select(choices=Event.objects.all(), attrs={'class': "form-select"}))
    # presentator = forms.ModelChoiceField(label="Presentator", queryset=User.objects.all(), widget=forms.Select(choices=User.objects.all(), attrs={'class': "form-select"}), required=False, blank=False)
    # attendant = forms.ModelChoiceField(label="Attendant", queryset=User.objects.filter(groups__name='Attendant'), widget=forms.Select(choices=User.objects.filter(groups__name='Attendant'), attrs={'class': "form-select"}), required=False, blank=False)
    # title = forms.CharField(label='Title', max_length=100, widget=forms.TextInput(attrs={'class': "form-control"}),)
    # description = forms.CharField(label='Description (max. 2048 signs)', max_length=2048, widget=forms.Textarea(attrs={'class': "form-control"}),)
    # target_group = forms.CharField(label='Target Group', widget=forms.Select(choices=TARGET_GROUP, attrs={'class': "form-control"}))
    # qualification_for_lecture = forms.CharField(label='I am qualified for the presentatien beacause...', max_length=100, widget=forms.TextInput(attrs={'class': "form-control"}),)
    ...
    # related_website = forms.CharField(label='Lecture related Website', max_length=100, widget=forms.URLInput(attrs={'class': "form-control"}), required=False)
    # scheduled_in_room = forms.ModelChoiceField(label="Scheduled in Room", queryset=Room.objects.all(), widget=forms.Select(choices=Room.objects.all(), attrs={'class': "form-select"}), required=False)
